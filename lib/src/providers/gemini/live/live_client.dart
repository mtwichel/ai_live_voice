...
import '../models/config/live_config.dart';
import '../models/config/session_resumption_config.dart';
import '../models/messages/client/client_message.dart';
import '../models/messages/server/server_message.dart';
...
class LiveClient {
  ...
  Future<LiveSession> connect({
    ...
    try {
      ...
      await _closeSessionAfterFailedSetup(session);
      rethrow;
    } on Exception catch (e) {
  ...
  /// After a failed setup the server may have already closed the WebSocket.
  /// [LiveSession.close] can then throw (e.g. [WebSocketConnectionClosed]),
  /// which would mask the real setup failure unless cleanup is best-effort.
  Future<void> _closeSessionAfterFailedSetup(LiveSession session) async {
    try {
      await session.close();
    } catch (_) {
